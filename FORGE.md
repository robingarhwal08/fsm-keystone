...
## WO-008: User Story: WO-008 - Pin route-by-role authorization behaviour with MockMvc slices
- **Status:** completed
- **Commit:** `da9bf3c`
- **Files:** 25 (+1219/-0)
- **Duration:** 965ss
...
## WO-020: User Story: WO-020 - Dual-Key JWT Verification Window For Zero-Downtime Rotation
- **Status:** completed
- **Commit:** `3e405a9`
- **Files:** 12 (+929/-114)
- **Duration:** 1445ss
...
## WO-025: User Story: WO-025 - Author V1 baseline schema migration for eight existing tables
- **Status:** completed
- **Commit:** `6e9cbda`
- **Files:** 6 (+968/-4)
- **Duration:** 915ss
- **Approach:** Derived authoritative DDL from all 8 JPA entities in com.fsm.keystone.entity, mapping Hibernate type defaults to exact PostgreSQL equivalents (bigint generated by default as identity for GenerationType.IDENTITY, varchar(255) for @Enumerated(STRING) enums, numeric(38,2) for BigDecimal, float(53) for Double, timestamp(6) for LocalDateTime). Tables created in FK-safe order with explicitly named pk_/uq_/fk_ constraints. ON DELETE CASCADE applied where the JPA entity model uses CascadeType.ALL or orphanRemoval=true (sites, status_history, time_logs, part_usage); RESTRICT for all other FKs. Three test layers verify the schema: @DataJpaTest repository slices, a @SpringBootTest lifecycle IT, and a manual Flyway API baseline IT. Seed fixture uses IDs 601+ to avoid collision with existing test data.

## WO-051: User Story: WO-051 - Authorize all SiteController endpoints by role
- **Status:** completed
- **Commit:** `bb98c54`
- **Files:** 3 (+502/-1)
- **Duration:** 426ss
- **Approach:** Added @PreAuthorize annotations to all five SiteController handlers using Spring Security SpEL. Mutations (POST, PUT, DELETE) are MANAGER-only. GET /api/sites allows all four roles. GET /api/sites/customer/{customerId} uses a compound expression: MANAGER/DISPATCHER unconditional, CUSTOMER only when the path variable customerId equals authentication.principal.customer?.id (safe-navigation for null customer). @EnableMethodSecurity was already present on SecurityConfig so no infrastructure change was needed. For the byCustomer CUSTOMER ownership tests in @WebMvcTest, @WithMockUser was used for all simple role cases, and JWT + mocked UserDetailsService returning a full AppUser was used for the ownership tests (since the SpEL expression accesses customer.id which is only on AppUser, not on Spring Security's User stub). The integration test uses @SpringBootTest + Testcontainers with two programmatically-created customers to verify end-to-end 200/403 outcomes with a real AppUser principal.
